...
#'   most variable cpgs as predictors to fit elastic net, random forest or
...
#' @param predictMethod what prediction method to use(glmnet or randomForest
...
pipeMostVar <- function(rowNum, Beta_df, beta2M = TRUE, respCol_index,
                        designInfo_df, alphaValue = seq(0, 1, by = 0.1),
                        ncores = 2, npredictors = 5000,
                        predictMethod = c("glmnet", "randomForest", "svm"),
                        outcome_type = "binomial",
                        save = FALSE, resultPath = FALSE){
  ...
  ## compute sd
  stand.dev <- apply(Train_mat, 2, sd)
  std.sorted <- sort(stand.dev, decreasing = TRUE)

  ## 2. choose predictors
  predictors <- names(std.sorted)[seq_len(npredictors)]
  p <- length(predictors)

  ##  subset based on predictors ###########
  ...
  if(save){
    saveRDS(out_ls, paste0(resultPath,"var-", npredictors,"-",
